...
const isPublicRoute = (path: string) => {
  ...
  return routesWithoutAuth.some(route =>
...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    if (!token) {
      return NextResponse.redirect(new URL('/error?error=Unauthorized', request.url))
    }

    if (token.email && !validateEmail(token.email)) {
      return NextResponse.redirect(new URL('/error?error=UnauthorizedEmail', request.url))
    }
    ...
    if (
      ...
      return NextResponse.redirect(new URL('/error?error=HyderabadOnly', request.url))
    }
