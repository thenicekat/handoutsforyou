...
export default function HandoutsPerYear({ handouts, year, searchWord }: HandoutsPerYearProps) {
  return (
    <div className="p-3 m-3">
      {
        handouts
          .filter((handout: any) => handout.toLowerCase().includes(searchWord.toLowerCase())).length > 0
          ?
          handouts
            .filter((handout: any) => handout.toLowerCase().includes(searchWord.toLowerCase()))
            .map((handout: any) => (
              <div key={handout} className='m-2 py-1 rounded-xl'>
                <div role="alert" className="alert">
                  ...
                  <span className="text-ellipsis">{handout.toUpperCase().replace("-", " ").replace("_", " ").split(".")[0]}</span>
                  <div>
                    <button className="btn btn-sm btn-ghost">{'Year ' + year.split("_")[0] + '-' + year.split("_")[1] + " " + (year.split("_").length > 2 ? year.split("_")[2] : "")}</button>

                    <Link href={"https://github.com/thenicekat/handoutsforyou/raw/main/public/handouts/" + year + '/' + handout + '?raw=true'} target="_blank"><button className="btn btn-sm" tabIndex={-1}>View</button></Link>
                  </div>
                </div>
              </div>
            )) :
