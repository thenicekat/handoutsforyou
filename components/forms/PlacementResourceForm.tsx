...
const placementResourceSchema = z.object({
    ...
    category: z
        ...
        .refine(
            (val) => zobCategories.includes(val),
            'Please select a valid category from the list'
        ),
})

export type PlacementResourceFormData = z.infer<typeof placementResourceSchema>

interface PlacementResourceFormProps {
    onSubmit: (data: PlacementResourceFormData) => void
    isLoading?: boolean
    defaultValues?: Partial<PlacementResourceFormData>
}
...
export default function PlacementResourceForm({
    ...
    const {
        ...
        formState: { errors },
        reset,
    } = useForm<PlacementResourceFormData>({
        resolver: zodResolver(placementResourceSchema),
        defaultValues: {
            name: '',
            link: '',
            createdBy: '',
    ...
    const categoryOptions = zobCategories.map((category) => ({
        value: category,
        label: category,
    }))
