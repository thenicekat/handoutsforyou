...
export default async function handler(
    ...
    if (!testimonial) {
        res.status(422).json({
            message: 'Invalid Request - testimonial missing',
            error: true,
        })
        return
    }
    ...
    if (!name || !email) {
        res.status(422).json({
            message: 'Invalid Request - Auth missing',
            error: true,
        })
        return
    }
    ...
    try {
        const response = await fetchWithRetry(
            'https://yearbooknostalgia.com/portal/write-testimonial-post',
            {
                method: 'POST',
                headers: {
                    Accept: '*/*',
                    ...
                    Referer:
                        'https://yearbooknostalgia.com/portal/testimonial-post?user_id=0&user_name=Name&user_email=email',
                    'Sec-CH-UA-Mobile': '?0',
